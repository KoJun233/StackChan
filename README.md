...
- 管理员导入且默认停用的完整 Skill ZIP、七个内置只读 Tool（按设备和授权范围开放）、页面管理且加密认证的 Streamable HTTP MCP、调用预算和隐私安全审计。
...
- 严格隔离的多角色容器、可选角色 TTS 音色、经敏感过滤且确认后生效的长期记忆建议、有界相关检索、单次/周期提醒、有限主动问候和默认关闭的无声陪伴表情。
