...
defineOptions({ name: 'PersonalTaskDetail' })
...
function onSubmit() {
  formRef.value?.submit().then((success) => {
    ...
    eventBus.emit('get-personal-task-list')
    onCancel()
  })
}

function onCancel() {
  router.back({ name: 'personalTaskList' })
}
</script>
...
<template>
  <div>
    <FaFixedBar position="top" class="p-0">
      <FaPageHeader :title="route.params.id ? '编辑待办' : '新增待办'" class="mb-0 border-b-none">
