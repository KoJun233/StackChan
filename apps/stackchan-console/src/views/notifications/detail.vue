...
<template>
  <div>
    <FaFixedBar position="top" class="p-0">
      <FaPageHeader :title="route.params.id ? '编辑通知集成' : '新增通知集成'" class="mb-0 border-b-none">
    ...
    <FaPageMain>
      <div class="max-w-4xl w-full">
        <DetailForm ref="formRef" :id="(route.params.id as string) || ''" />
      </div>
    </FaPageMain>
    <FaFixedBar position="bottom" class="flex-center gap-4">
      <FaButton @click="onSubmit">提交</FaButton>
      <FaButton variant="outline" @click="onCancel">取消</FaButton>
    </FaFixedBar>
  </div>
</template>
