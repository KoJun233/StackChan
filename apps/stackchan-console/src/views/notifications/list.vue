...
const activeSection = ref<'integrations' | 'queue' | 'test'>('integrations')
const sectionTabs = [
  { label: '接入配置', value: 'integrations', icon: 'i-ri:plug-line' },
  { label: '测试播报', value: 'test', icon: 'i-ri:volume-up-line' },
  { label: '通知队列', value: 'queue', icon: 'i-ri:list-check-3' },
]
...
const testResponseActionOptions: { label: string, value: NotificationResponseAction }[] = [
  { label: '已知晓', value: 'ACKNOWLEDGE' },
  { label: '稍后提醒', value: 'SNOOZE' },
  { label: '标记完成', value: 'COMPLETE' },
]
  ...
  useFaModal().confirm({
    ...
    onConfirm: async () => {
      ...
      try {
        ...
        if (search.value.integrationId === integration.id) {
          search.value.integrationId = ''
        }
  ...
  <div :class="{ 'absolute flex flex-col size-full': tableAutoHeight }">
    ...
    <FaModal v-model="tokenModalOpen" title="签发外部通知令牌" :footer="false" @closed="closeTokenModal">
      <div v-if="!issuedToken" class="space-y-4">
        ...
        <FaLabel label="到期时间（可选，最长一年）">
          <FaInput v-model="tokenExpiresAtLocal" type="datetime-local" />
        </FaLabel>
        <div class="flex gap-2 justify-end">
          <FaButton variant="outline" @click="closeTokenModal">
            取消
          </FaButton><FaButton :loading="actionId === tokenIntegration?.id" @click="issueToken">
            签发令牌
          </FaButton>
        </div>
      </div>
      <div v-else class="space-y-4">
        ...
        <div class="text-sm text-muted-foreground">
          到期：{{ formatTime(issuedToken.metadata.expiresAt) }}
        </div>
        <div class="flex gap-2 justify-end">
          <FaButton @click="copyIssuedToken">
            <FaIcon name="i-ri:file-copy-line" />复制令牌
          </FaButton><FaButton variant="outline" @click="closeTokenModal">
            我已安全保存
          </FaButton>
        </div>
      </div>
