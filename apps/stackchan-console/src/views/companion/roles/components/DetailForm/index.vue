...
const model = ref<RoleFormModel>({ name: '', tone: 'WARM', replyLength: 'BALANCED', proactivity: 'BALANCED', backgroundInstructions: '', topicBoundaries: '', taboos: '', ttsVoiceOverride: '' })
const schema = toTypedSchema(z.object({ name: z.string().trim().min(1).max(80), tone: z.enum(['WARM', 'CALM', 'LIVELY', 'PROFESSIONAL']), replyLength: z.enum(['SHORT', 'BALANCED', 'DETAILED']), proactivity: z.enum(['RESERVED', 'BALANCED', 'PROACTIVE']), backgroundInstructions: z.string().max(4000), topicBoundaries: z.string().max(2000), taboos: z.string().max(2000), ttsVoiceOverride: z.string().trim().max(160).nullable() }))
...
async function load(id: string) {
  if (!id) return
  loading.value = true
  try { const role = await getRole(id); model.value = { name: role.name, tone: role.tone, replyLength: role.replyLength, proactivity: role.proactivity, backgroundInstructions: role.backgroundInstructions, topicBoundaries: role.topicBoundaries, taboos: role.taboos, ttsVoiceOverride: role.ttsVoiceOverride ?? '' } }
...
async function submit() {
  ...
  loading.value = true
  try { const input = { ...model.value, name: model.value.name.trim(), backgroundInstructions: model.value.backgroundInstructions.trim(), topicBoundaries: model.value.topicBoundaries.trim(), taboos: model.value.taboos.trim(), ttsVoiceOverride: model.value.ttsVoiceOverride.trim() || null }; if (props.id) await updateRole(props.id, input); else await createRole(input); useFaToast().success(props.id ? '角色已更新' : '角色已创建'); return true }
