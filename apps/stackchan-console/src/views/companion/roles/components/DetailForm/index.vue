...
const model = ref<CompanionRoleInput>({ name: '', tone: 'WARM', replyLength: 'BALANCED', proactivity: 'BALANCED', backgroundInstructions: '', topicBoundaries: '', taboos: '' })
const schema = toTypedSchema(z.object({ name: z.string().trim().min(1).max(80), tone: z.enum(['WARM', 'CALM', 'LIVELY', 'PROFESSIONAL']), replyLength: z.enum(['SHORT', 'BALANCED', 'DETAILED']), proactivity: z.enum(['RESERVED', 'BALANCED', 'PROACTIVE']), backgroundInstructions: z.string().max(4000), topicBoundaries: z.string().max(2000), taboos: z.string().max(2000) }))
...
async function load(id: string) {
  if (!id) return
  loading.value = true
  try { const role = await getRole(id); model.value = { name: role.name, tone: role.tone, replyLength: role.replyLength, proactivity: role.proactivity, backgroundInstructions: role.backgroundInstructions, topicBoundaries: role.topicBoundaries, taboos: role.taboos } }
...
async function submit() {
  ...
  loading.value = true
  try { const input = Object.fromEntries(Object.entries(model.value).map(([key, value]) => [key, typeof value === 'string' ? value.trim() : value])) as unknown as CompanionRoleInput; if (props.id) await updateRole(props.id, input); else await createRole(input); useFaToast().success(props.id ? '角色已更新' : '角色已创建'); return true }
...
<template>
  <FaLoading :loading="loading"><FaForm ref="formRef" :model="model" :validation-schema="schema" :label-width="120" class="grid gap-6" scroll-to-error>
    ...
    <FaAlert title="严格隔离" description="新角色会拥有独立的网页/语音会话、长期记忆、提醒和主动话题状态；音量、免打扰、模型和语音供应商仍按设备共享。" />
  </FaForm></FaLoading>
</template>
