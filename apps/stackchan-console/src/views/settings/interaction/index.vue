...
async function loadSettings(deviceId: string) {
  ...
  try {
    ...
    await loadCalendarEvents(deviceId, calendar.configured)
...
async function loadCalendarEvents(deviceId = model.value.deviceId, configured = icloudConnection.value?.configured) {
  ...
  try {
    ...
    calendarEvents.value = await getICloudCalendarEvents(deviceId, from.toISOString(), to.toISOString())
  }
  catch (error) {
    calendarEvents.value = []
  ...
  <AppPageShell :title="pageTitle" :description="pageDescription">
    <FaLoading :loading="loading">
      <FaForm
        ...
        @submit="submit"
      >
        <FaCard v-if="isWorkdayPage" title="当前机器人" description="工作状态、日历天气和观察数据都按机器人隔离。">
          <FaFormItem name="deviceId" label="目标机器人" required class="max-w-xl">
              ...
              <div class="gap-6 grid">
                ...
                <div class="pt-6 border-t gap-5 grid">
                  ...
                  <div v-if="icloudConnection?.configured" class="pt-5 border-t gap-3 grid">
                    ...
                    <FaLoading :loading="calendarEventsLoading">
                      <FaEmpty v-if="!calendarEvents.length" description="未来 7 天暂无已缓存日程" />
        ...
        <FaFixedBar position="bottom" class="flex justify-center">
