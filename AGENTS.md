...
- 实现、状态更新和交接记录必须在创建 PR 前压缩到同一个任务提交中，不保留独立交接提交。
- 运行 `git diff --check` 和 `pnpm docs:check`。
...
- 开发过程中可以产生多个临时提交，但创建 PR 前必须将任务分支的全部提交压缩为一个提交，并重新完成该提交的验证。
- PR 标题必须遵循与提交信息相同的中文 Conventional Commit 规则，目标分支必须为 `master`。
- Agent 只能创建 PR，不得自行合并；必须等待人工审核并由人工执行合并。
- `master` 禁止 force push。任务分支因压缩提交需要改写远端历史时，只允许使用 `git push --force-with-lease`。
- 合并时使用 merge commit，保留“一个任务提交 + 一个 PR 合并提交”的审计链路；不使用 squash merge 或 rebase merge。
- GitHub 仓库规则必须要求 PR、人工审核和 PR 策略检查通过，并禁止管理员或其他角色绕过规则。
