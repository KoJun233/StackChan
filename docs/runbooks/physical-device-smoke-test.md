...
6. For an INT-013 candidate, ask for a benign reply containing at least three sentences. Confirm playback starts before the full reply would previously have finished synthesizing, all sentences play once in order, and the device reports one `PLAYBACK_STARTED` followed by one `PLAYBACK_COMPLETED`. Compare the privacy-safe `UPLOAD_STARTED -> PLAYBACK_STARTED` duration with an SCV1 baseline; do not retain audio, transcript or reply text as evidence.
7. During the first segment of another multi-sentence reply, touch to cancel. Confirm playback stops, no later segment plays, the turn ends as `CANCELLED`, and a new wake or press-to-talk turn completes normally without any fragment from the cancelled reply.
8. Verify rolling compatibility separately: current server with an SCV1 request still returns and plays a complete SCV1 body; current firmware against a server without SCV2 support accepts the negotiated SCV1 fallback. Do not downgrade or flash another image solely for this check without separate authorization.
6. Leave the device untouched for 300 seconds. Confirm the display changes to low brightness only while idle or offline and only the two pupils move every 2.5 seconds; the background, eye whites, mouth and status indicator must not periodically redraw or visibly flash. Touch the screen and confirm normal brightness and the current face return immediately.
7. In the reminder management page, create a one-time benign reminder for the online test device a few minutes in the future. Let the device enter the screensaver first. At the scheduled time, confirm it exits the screensaver, downloads only the fixed same-origin reminder path, speaks the reminder, shows success, sends `accepted=true`, and the reminder becomes `DELIVERED`.
8. Repeat with the device offline at the scheduled time. Confirm the reminder remains `PENDING`, reconnect the device, then confirm it is dispatched, spoken, acknowledged once, and becomes `DELIVERED`.
9. During all checks, confirm no actuator initializes or moves and `safety_state=motion_disabled` remains unchanged.
