...
2. Say the wake phrase expected by the current image, then a short benign request. Confirm the face changes to listening, recording ends after local silence or at the eight-second maximum, the face changes to processing, and the speaker plays the synthesized reply with the speaking face. Confirm a short green success face appears before idle and a second wake phrase works, proving microphone listening resumed after playback.
3. Complete one wake without valid speech. Confirm the device shows the gentle cyan no-speech face rather than the recoverable-error face, returns to idle, and accepts the next wake.
4. Temporarily make the configured speech provider unavailable without exposing its credentials. Confirm the device shows the orange recoverable-error face briefly, returns to idle, remains `motion_disabled`, and can complete another turn after the provider is restored.
5. Interrupt Wi-Fi or the authenticated WebSocket. Confirm the gray offline face persists rather than timing out to idle, then restore connectivity and confirm the current usable state returns automatically.
6. For an ADR-0049 live-upload candidate, deploy the matching server first, then complete two independent wakes with normal speech. Confirm no `/voice/turn/live` request appears before local VAD, and each accepted turn logs `Voice live upload opened` followed by `Voice live upload completed` with the expected bounded `pcm_bytes` and a `capture_tail_ms` near the 100 ms target. Confirm the server receives the live endpoint, no legacy retry follows a completed live body, both replies play, WakeNet resumes, and internal SRAM/PSRAM diagnostics show no reset or allocation failure. Record only timing, sizes, safe heap diagnostics and result; do not retain audio or text.
7. For an INT-013 candidate, ask for a benign reply containing at least three sentences. Confirm playback starts before the full reply would previously have finished synthesizing, all sentences play once in order, and the device reports one `PLAYBACK_STARTED` followed by one `PLAYBACK_COMPLETED`. Compare the privacy-safe `UPLOAD_STARTED -> PLAYBACK_STARTED` duration with an SCV1 baseline; do not retain audio, transcript or reply text as evidence.
8. During the first segment of another multi-sentence reply, touch to cancel. Confirm playback stops, no later segment plays, the turn ends as `CANCELLED`, and a new wake or press-to-talk turn completes normally without any fragment from the cancelled reply.
9. Verify rolling compatibility separately: current server with an SCV1 request still returns and plays a complete SCV1 body; current firmware against a server without SCV2 support accepts the negotiated SCV1 fallback. Do not downgrade or flash another image solely for this check without separate authorization.
10. Leave the device untouched for 300 seconds. Confirm the display changes to low brightness only while idle or offline and only the two pupils move every 2.5 seconds; the background, eye whites, mouth and status indicator must not periodically redraw or visibly flash. Touch the screen and confirm normal brightness and the current face return immediately.
11. In the reminder management page, create a one-time benign reminder for the online test device a few minutes in the future. Let the device enter the screensaver first. At the scheduled time, confirm it exits the screensaver, downloads only the fixed same-origin reminder path, speaks the reminder, shows success, sends `accepted=true`, and the reminder becomes `DELIVERED`.
12. Repeat with the device offline at the scheduled time. Confirm the reminder remains `PENDING`, reconnect the device, then confirm it is dispatched, spoken, acknowledged once, and becomes `DELIVERED`.
13. During all checks, confirm no actuator initializes or moves and `safety_state=motion_disabled` remains unchanged.
