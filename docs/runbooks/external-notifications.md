...
- 在集成编辑页将“摘要聚合”设为 5–300 秒；填 0 关闭。新建集成默认关闭，避免改变已有调用方延迟。
- 只有同一集成、设备、角色的单向通知会聚合，每批最多 10 条、固定编号并保留原文；互动通知始终逐条播报。
- 外部调用方继续逐条保存和轮询通知 ID。摘要播放时成员均显示 `DISPATCHED`，同一设备 ACK 后同时进入相同终态。
- 聚合窗口会增加首条通知的等待时间。需要立即播放或用户回执时，关闭聚合或为通知声明 `responseActions`。
