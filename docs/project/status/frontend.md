...
在主动关心页提供独立的无声陪伴开关、清晰边界和下一候选时间，同时保持默认关闭。

## 已完成并发布的 COMPANION-003

- “主动陪伴”卡片增加无声陪伴开关，明确每 30–90 分钟、每天最多八次，并复用下方允许时段。
- 页面展示保存后的下一候选时间，并说明离线、免打扰、语音和提醒播放期间不会显示。
- 文案明确该能力不播音、不调用模型、不占主动问候次数，也不读取摄像头、麦克风或电脑活动。
- 交互 API 类型和保存载荷覆盖 V49 字段；控制台 32 文件 102/102、类型检查、production build、定向 ESLint/Stylelint 和运行静态资源验证通过。
