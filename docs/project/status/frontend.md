...
将“人设设置”升级为 Fantastic-admin 角色 CRUD，提供设备角色绑定，并为聊天、记忆、提醒、通知集成和个人数据增加角色选择/筛选。
...
无代码实现项；角色管理页面已部署到 LAN，等待用户验收。

## 下一步操作

执行管理员页面、新会话绑定和设备角色切换人工复核。

## 阻塞项

- 当前无实现或部署阻塞；等待用户完成管理员浏览器复核。
...
- Node v24.19.0、pnpm 11.19.0：Vitest 25 个文件 69/69、`vue-tsc -b` 和 production build 通过；路由专项包含在全量测试中。
