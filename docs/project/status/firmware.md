...
维持已验收的 CoreS3 固件运行态。`EVT-001` 已复用现有提醒播放、WebSocket 和 ACK 能力，未修改固件协议或镜像；`ROLE-001` 同样不应写入固件。
...
保持设备不变。EVT-001 server 已部署，只需执行现有 `speak_reminder` 在线、免打扰延期和离线重连人工验收；ROLE-001 的角色选择由服务端路由，不写入固件。
