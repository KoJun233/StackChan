...
- 基准提交：`06ab0b3`
- 最后验证提交：`06ab0b3`
- 最后验证范围：COMPANION-003 定向 25/25；排除 8 个既有 Windows loopback 类后的服务端 482/482；完整 514 个用例中 31 个错误均来自这些环境限制；V49 空库迁移通过

## 当前目标

提供独立、低打扰且默认关闭的无声陪伴表情；持久化随机候选，并在语音、提醒、免打扰或离线时安全抑制。

## 已完成并发布的 COMPANION-003

- V49 保存独立开关、下一候选、上次成功时间和本地日期计数；与语音主动问候的三次额度完全分离。
- 开启后复用设备时区和允许时段，每 30–90 分钟随机候选，每个本地日期最多八次；重启恢复既有候选。
- 只下发当前角色主题下 6–8 秒的 `CONTENT/HAPPY/FOCUSED/TIRED` 弱或中等表情；不调用模型、不合成音频、不创建提醒，也不执行身体动作。
- 离线、免打扰、最近活动语音和正在播放的提醒会抑制显示；仅成功下发后记录次数和下一候选，同进程调度串行且数据库更新使用行锁。
- 定向 25/25、排除 8 个既有 Windows loopback 类后的服务端 482/482、V1..V49 空库迁移、备份恢复和 LAN 发布通过；运行设置保持默认关闭。

## 已完成并发布的 COMPANION-002

- 后台每三十分钟读取 Hacker News 官方只读 API，缓存最多两小时；只接受四天内、未删除且使用公开 HTTPS 链接的 story，不发送任何用户数据。
- 只有主动个性化开启且存在允许主动提及的确认记忆时，才把最多六个不可信候选标题交给模型；模型只返回候选编号和短开场，程序校验后确定性引用标题，播报最多保留标题前六十个码点。
- 无匹配、外部超时、模型失败、未知编号或不合规输出全部回退普通人设问候；主动生成路径只读缓存，不等待网络。
- V48 为主动提醒保存来源名、原始标题、链接、Hacker News 条目收录时间和抓取时间；同一设备不重复使用同一链接，用户编辑提醒时清除来源字段。
- 定向 18/18、排除 8 个既有 Windows loopback 类后的服务端 477/477、V48 空库迁移、备份恢复和 LAN 发布通过。
- 最终容器 `a145b96ea279` 运行镜像 `sha256:085ab90c9602e16789bdfd53f1f281d7695fc21d2454514dca607f21c6f92e7c`；启动后刷新成功缓存 12 条候选，容器无重启，主动设置和设备安全状态未改变。

## 已完成的 COMPANION-001

- V47 增加 `proactive_next_at`；启用或保存配置时在剩余允许窗口内抽取并持久化下一候选，成功占用名额后按至少一小时间隔重新抽取，达到三次上限后移动到下一本地日期。
- 调度器重启只为缺失候选的历史启用配置补计划，不反复重抽；到点后继续检查在线、免打扰、活动语音和待播放提醒。
- 主动措辞读取当前活动角色的人设字段；没有兴趣记忆时也按人设生成，失败才回退管理员保存的备用问候。
- 自动记忆提取只对用户明确表达的兴趣、爱好或内容偏好预置主动提及，待确认和停用边界不变；确认后相邻话题无需逐项确认。
- 输出策略拒绝没有来源的近期发布、新闻、模型、版本、比赛或价格断言；当前 DeepSeek 官方接口不具备可核验来源搜索，因此不生成实时资讯。
- 定向 22/22、排除 8 个既有 Windows loopback 类后的服务端 470/470、V47 空库迁移与镜像构建通过；LAN 已运行 `companion-random-v47`。

## 已完成的 VOICE-001

- 八十码点以内且没有查询、搜索、最新消息、Tool、Skill 或 MCP 意图的设备语音使用低延迟 ChatClient；DeepSeek V4 Flash 延续关闭推理模式，网页和长输入不变。
- 时间、能力、日历、天气、待办、下一提醒和待确认记忆保留必需 Tool；明确外部能力请求继续进入 ReactAgent。
- 模型回答继续流式接收，但不在首句回调内同步合成。每个回合固定只发起一次 TTS 并下发一个音频段，标点停顿交给语音服务；正文优先摘要到 160 码点以内，模型超限时在最近完整句界安全收束，隐藏表情标记不会朗读。
- 完整正文继续持久化、解析表情并生成待确认记忆建议；取消和客户端断开继续抑制晚到分段。
- 当前 `qwen-tts-latest` 是非实时模型且无通用倍速参数，本轮不伪造句内语速优化；后续切换实时模型需单独验证。
- 严格单 WAV 相关 44/44 通过；排除既有八个 Windows loopback 类后的服务端 463/463 通过。
- 镜像 `voice-natural-ac6f386` 已只替换 LAN server；V46、本机与局域网健康、MCP 预热和设备在线通过，PostgreSQL、Redis、备份容器与固件未替换。
- 实体反馈确认首句响应已明显加快，但一轮回复仍拆成 3–5 个独立 TTS 请求；日志显示 flush 均为 0 ms，停顿主要来自每段重新起音、收尾和标点静音。
- 普通长度的第一句之后，剩余多句现在合并为一个连续 TTS 段；超过 160 字才继续安全拆分。闲聊提示进一步限制为尽量不超过四十个汉字，专项测试更新为 36/36。
- 连续余句镜像 `voice-natural-pause-v2` 已发布；本机与 LAN 健康、V46、MCP 预热和设备在线通过，等待一轮实体停顿复测。
- “现在几点”实体回合显示 ASR 448 ms、Agent/Tool 4,673 ms、首段在请求后 5,262 ms 发出，且一句话因逗号拆成 14/12 字两段；第二段另需 1,684 ms 合成。播放结束后的无人续听被 ASR HTTP 400 误归类为服务错误，触发红色错误表情。
- 语音时间问题现在直接执行已授权的 `current_date_time` 并生成一个完整中文句子，不调用模型；网页时间查询和其他事实能力保持受控 Agent。首段不再按逗号切分，普通单句整体合成。
- DashScope 明确返回官方无有效语音或无词错误时转换为 `NO_SPEECH`；其他 400 与配置错误仍按服务失败处理。镜像 `voice-time-single-v3` 已发布，本机与 LAN 健康、V46、MCP 预热和设备在线通过。
- 用户纠正关注点为两段音频之间约两秒等待，而非整体调用耗时。代码复核确认首句 TTS 在模型流回调中同步执行，阻塞后续文本消费；首段发出后余句才开始另一轮 TTS。当前修复移除这条短首段路径，普通回复合成为一份 WAV，从结构上消除服务端段间合成等待。
- 连续单段相关 41/41、排除 8 个既有 Windows loopback 类后的服务端 464/464 通过；镜像 `voice-continuous-v4` 已发布，本机与 LAN 健康、V46、MCP 预热和设备在线通过。
- 用户确认不需要保留超长回复分段。业务分段器已删除，所有语音回复固定为 sequence 0 和 segmentCount 1；SCV2 的八段协议上限移入封装层，仅作为兼容边界保留。超限自然收束与单段协议定向 12/12 通过。
- `voice-single-wav-v5` 已只替换 LAN server；本机与局域网健康、V46、MCP 预热和设备在线通过。标点停顿由 `qwen-tts-latest` 在同一次合成内处理，等待用户实体听感复测。
...
- 新增 `GET /api/v1/workday/{deviceId}/calendar/events`，复用既有设备校验、缓存过期清理、最多七天窗口和私密日程脱敏边界。
