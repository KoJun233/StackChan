...
- 基准提交：`efcb85d`
- 最后验证提交：`efcb85d`
- 最后验证范围：COMPANION-001 定向 22/22；排除 8 个既有 Windows loopback 类后的服务端 470/470；完整 502 个用例中 31 个错误均来自这些环境限制；V47 空库迁移和镜像构建通过

## 当前目标

在保留现有可靠主动投递、免打扰、忙碌仲裁和角色隔离的同时，把主动闲聊改为持久随机时刻，并让开场读取当前人设和确认兴趣。无可信来源时禁止生成实时资讯。
