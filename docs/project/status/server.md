...
- 基准提交：`c2b0fde`
- 最后验证提交：`ac6f386`
- 最后验证范围：VOICE-001 严格单 WAV 相关 44/44；排除既有 8 个 Windows loopback 类后的服务端 463/463；镜像构建、备份恢复与 LAN 发布通过

## 当前目标

在保留 V46、live 上传、SCV2 顺序/取消和隐私边界的同时，让高频闲聊保持快速连续，让本地可确定的时间问题跳过 ReactAgent，并让连续对话无人回应时安静结束。LAN 已发布，下一步复测两句话之间没有额外空档以及结束表情。
