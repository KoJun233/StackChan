...
`EVT-001`、`ROLE-001`、`ROLE-002` 与 `EVT-002` 已分别通过 PR #17–#20 合入 `master`。用户要求继续暂停连接器，当前进入 `EVT-003` 确定性通知摘要聚合。

当前 LAN 仍运行 ROLE-002 `b6cad0b` server/V30；EVT-002/EVT-003 尚未部署，固件未修改。

## 工作流摘要

| 工作流 | 状态 | 当前事实 | 下一步 |
| --- | --- | --- | --- |
| [服务端](server.md) | READY_FOR_REVIEW | V32 集成级等待窗口、确定性聚合和投递组 ACK 已验证 | 获授权后部署人工验收 |
| [前端](frontend.md) | READY_FOR_REVIEW | 集成表单和列表可配置/展示摘要窗口 | 获授权后部署人工验收 |
| [固件](firmware.md) | STABLE | CoreS3 `7e7c55f / motion_disabled / OTA=true` | 后续两任务不改固件 |
| [部署](deployment.md) | STABLE | LAN 仍运行 ROLE-002 server/V30 | 获明确授权后再部署 EVT-003 |
...
- Git：`master` 合并提交 `6ed3b5d`；当前任务分支基于该提交。
...
- EVT-003 当前工作树基于 `6ed3b5d`，实现与交接已完成，等待整理单一任务提交。
- EVT-003 工作树：服务端 367/367 及空 PostgreSQL Flyway V1..V32 通过；前端 Node v24.19.0、pnpm 11.19.0 下 25 个文件 70/70、`vue-tsc -b` 和 production build 通过。
...
- `EVT-001`、`ROLE-001`、`ROLE-002`、`EVT-002` 已合入；EVT-003 无前置阻塞。
...
完成 EVT-003 状态交接与提交级静态检查，整理为单一中文任务提交；随后等待用户决定是否部署。未经明确授权不部署、不推送、不连接 COM3、不刷写固件。
