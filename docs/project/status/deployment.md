...
- 状态：STABLE
- 最后更新：2026-08-31
- 当前分支：`codex/work-005-daily-task-brief`（运行态为 WORK-005 server/V45，实机固件为 `424cb49`）
- 基准提交：`861d26a`
- 最后验证提交：`861d26a`
- 最后验证范围：WORK-005 新备份、独立恢复、server-only 发布、V45、健康、LAN、观察窗口与设备安全状态通过
- 当前模式：LAN HTTP development

## 当前目标

保持已发布的 WORK-005/V45 首次简报待办摘要稳定运行，并在下一次合法首次简报完成实体语音验收；CoreS3 继续保持 `424cb49 / motion_disabled`。
...
LAN server 已运行 WORK-005/V45，当前宿主机局域网地址为 `http://192.168.1.4:8080/`，容器为 `e384ffebb23a`，镜像摘要为 `sha256:f9e5db38787621ea85d26d302fc92a0d6b5aee6f4051c85b43c1d460d9675112`。运行态提供本地待办闭环，并把当前角色最多两项优先待办加入每日首次简报。
...
下一工作日显式开始工作模式，确认已有当前角色待办按规则进入首次简报且不包含备注。当天已有首次简报时不重置观察窗口或绕过去重；Git 外部推送仍需用户授权。
