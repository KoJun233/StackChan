...
- WakeNet 短帧的完成检测不等待显示/UI 互斥锁。录音请求入队后，高优先级语音任务先阻塞一个 FreeRTOS tick，让底层麦克风 I/O 处理请求；重新运行时若录音状态已归零，应视为短帧已经成功完成，而不是要求必须观察到极短的“正在录音”中间状态。后续仍在录音时按 tick 轮询并保留超时上限，既避免误报 `ESP_ERR_TIMEOUT`，也防止挤占 CPU0 idle task。`MEDIA-002D` 将底层实现从 M5Unified 迁移到官方 BSP `esp_codec_dev`，本时序约束保持不变。
