...
捆绑运行时会更新，使用前必须以 `node --version` 核实，不依赖文档中的历史版本。2026-09-13 复核为 v24.19.0，满足当前 `engines`；旧 v24.14.0 的“仅可做文档恢复检查”结论已过时。前端测试、构建和开发仍必须满足仓库 `engines`（`^22.22.2 || ^24.15.0 || >=26.0.0`），不得削弱版本约束。本任务捆绑 pnpm 报告 11.19.0，仅用于执行已安装依赖的脚本，未安装依赖或改动锁文件；依赖安装与可复现工具链仍以 `packageManager` 指定的 11.13.1 为准，交接中应保留这一脚本启动器版本差异。
